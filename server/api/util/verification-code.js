...
module.exports.sendVerificationEmail = async (user, apiUrl) => {
  const COMPANY_NAME = `Oasys I.T. Solutions`;
  const email = user.email;
  const code = crypto.randomBytes(20).toString('hex');
  const nonce = crypto.randomBytes(10).toString('hex');
  const url = `${apiUrl}api/users/verify?_key=${nonce}&_type=${VERIFICATION_CODE_TYPES.EMAIL_VERIFICATION}&_code=${code}`;
...
module.exports.sendPasswordResetEmail = async (user, apiUrl) => {
  const COMPANY_NAME = `Oasys I.T. Solutions`;
  const email = user.email;
  const code = crypto.randomBytes(20).toString('hex');
  const nonce = crypto.randomBytes(10).toString('hex');
  const url = `${apiUrl}api/users/reset-password?_key=${nonce}&_type=${VERIFICATION_CODE_TYPES.FORGOT_PASSWORD}&_code=${code}`;
  await VerificationCode.create({
    email: user.email,
    userId: user.id,
